...
const handleConfirmButton = async (event) => {
    ...
    if (!engineName.value) {
        alerts.push("You need to provide a name!");
    }
    let urlForm;
    try {
        ...
        alerts.push("You need to provide a valid URL!");
    }
    if (alerts.length >= 1) {
        alert(alerts.join("\n"));
        return;
    ...
    try {
        let resp = await fetch(  // Export data to https://paste.mozilla.org
        ...
        document.getElementById("confirmation-popover").classList = [];
        engineName.value = "";
        engineUrl.value = "";
        engineDescription.value = "";
        await sleep(5);  // Show confirmation popover
        document.getElementById("confirmation-popover").classList = ["hidden"]
    } catch (error) {
...
const handleAutofillButton = async (event) => {  // Extract a destination and autofill it
    let ret = await browser.tabs.executeScript({ code: "extractDest();" });
    engineUrl.value = ret;
}

const handleToggleDataNotice = (event) => {  // Show/hide the privacy stuff
...
(async () => {
    ...
    if (elementId) {  // Don't autofill if we aren't being opened from a context menu
        let ret = await browser.tabs.executeScript({ code: "extractDest(" + elementId + ");" });
        engineUrl.value = ret;
    }
})();
